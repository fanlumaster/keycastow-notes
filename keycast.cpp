...
ATOM MyRegisterClassEx(HINSTANCE hInst, LPCWSTR className, WNDPROC wndProc) {
  WNDCLASSEX wcl;
  wcl.cbSize = sizeof(WNDCLASSEX); // 定义结构体的大小
  wcl.hInstance = hInst;           // 指定窗口类所属的实例句柄
  wcl.lpszClassName = className;   // 指定窗口类名称
  wcl.lpfnWndProc = wndProc;       // 指定绑定的窗口过程
  wcl.style = CS_DBLCLKS;          // 表示窗口类可以处理双击消息
  wcl.hIcon = LoadIcon(NULL, IDI_APPLICATION); // 使用默认应用程序图标
  wcl.hIconSm =
      LoadIcon(NULL, IDI_WINLOGO); // 使用默认的 win 窗口左上角的小图标
  wcl.hCursor = LoadCursor(NULL, IDC_ARROW); // 使用标准的箭头光标
  wcl.hbrBackground =
      (HBRUSH)GetStockObject(WHITE_BRUSH); // 设置窗口的背景画刷为白色画刷
  wcl.lpszMenuName = NULL;                 // 指定窗口类没有菜单
  wcl.cbWndExtra = 0; // 指定窗口类实例额外分配的内存大小
  wcl.cbClsExtra = 0; // 指定窗口类额外分配的内存大小

  return RegisterClassEx(&wcl); // 注册窗口类
}
...
int WINAPI WinMain(HINSTANCE hThisInst, HINSTANCE hPrevInst, LPSTR lpszArgs,
  ...
  hInstance = hThisInst; // 当前实例的句柄，这个是由系统分配的

  INITCOMMONCONTROLSEX
  icex; // init common control sex, 指定在应用程序中启用哪些通用控件

  icex.dwSize = sizeof(INITCOMMONCONTROLSEX); // 定义结构体的大小
  icex.dwICC = ICC_LINK_CLASS                 // 链接控件
               | ICC_LISTVIEW_CLASSES         // 列表试视图控件
               | ICC_PAGESCROLLER_CLASS       // 页滚动控件
               | ICC_PROGRESS_CLASS           // 进度条控件
               | ICC_STANDARD_CLASSES // 标准通用控件，如按钮、组合框等
               | ICC_TAB_CLASSES      // 选项卡控件
               | ICC_TREEVIEW_CLASSES // 树状图控件
               | ICC_UPDOWN_CLASS     // 上下箭头控件
               | ICC_USEREX_CLASSES   // 用户自定义扩展的通用控件
               |
               ICC_WIN95_CLASSES // 用于向后兼容性，包括一些旧版 Windows 的控件
      ;
  InitCommonControlsEx(&icex); // 初始化

  /*
   * NULL 表示获取当前执行程序的路径
   * iniFile 接受文件路径的缓冲区
   * MAX_PATH 是系统定义的一个常量其值为 260
   */
  GetModuleFileName(NULL, iniFile, MAX_PATH); // 获取当前执行程序的文件路径
  iniFile[wcslen(iniFile) - 4] = '\0';  // 将 .exe 这四个字符截断
  wcscat_s(iniFile, MAX_PATH, L".ini"); // 将 .ini 拼接到 iniFile 的后面

// 在 debug 模式下进行一些信息的记录
#ifdef _DEBUG
  ...
  GdiplusStartupInput
      gdiplusStartupInput; // 声明一个结构体，后面要用来初始化 GDI+ 库
  ULONG_PTR
  gdiplusToken; // 声明 gdi+ 令牌，后续可以标识应用程序和 gdi+ 库之间的连接
  GdiplusStartup(&gdiplusToken, &gdiplusStartupInput, NULL); // 初始化 GDI+ 库

  // 注册窗口类
  // szWinName 是全局变量
  // WindowFunc 在前面定义过了
  // 这里进入 MyRegisterClassEx 看
  ...
  hMainWnd = CreateWindowEx(
      WS_EX_LAYERED | WS_EX_TOPMOST |
          WS_EX_NOACTIVATE, // 窗口具有透明度特性、置顶、不会获取焦点
      szWinName,            // 窗口类名
      szWinName,            // 窗口名
      WS_POPUP,             // 弹出式窗口，没有边框和标题栏
      0,                    // x
      0,                    // y
      1,                    // Width
      1,                    // Height
      NULL,                 // 父窗口
      NULL,                 // 菜单句柄
      hThisInst,            // 当前应用程序的实例的句柄
      NULL // 用户定义的一个参数，它会在窗口过程中收到
  );
